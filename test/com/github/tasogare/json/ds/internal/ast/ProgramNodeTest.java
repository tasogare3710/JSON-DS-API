...
public class ProgramNodeTest {
    ...
    public void test() {
        ...
        final TypeExpressionNode<TypeNameNode> aInitializer = new TypeExpressionNode<>(9, 15,
            newTypeName(9, 15, "string"));
        ...
        final TypeExpressionNode<TypeNameNode> bInitializer = new TypeExpressionNode<>(25, 32,
            newTypeName(25, 32, "number"));
        ...
        final ProgramNode<TypeDefinitionNode<? extends BasicTypeExpressionNode<?>>> p = new ProgramNode<>(0, 33, list,
            null);
    ...
    public void test2() {
        ...
        final TypeExpressionNode<ArrayTypeNode<TypeNameNode>> bInitializer = new TypeExpressionNode<>(25, 32,
            arrayType);
        final TypeDefinitionNode<ArrayTypeNode<TypeNameNode>> bDefinition = new TypeDefinitionNode<>(17, 33, B,
            bInitializer);
        ...
        final ProgramNode<TypeDefinitionNode<? extends BasicTypeExpressionNode<?>>> p = new ProgramNode<>(0, 33, list2,
            null);

        final List<TypeDefinitionNode<? extends BasicTypeExpressionNode<?>>> ds = p.getDirectives();
        for (final TypeDefinitionNode<? extends BasicTypeExpressionNode<?>> d : ds) {
            assertThat(d.getIdentifier().getString(), anyOf(equalTo("A"), equalTo("B")));
            assertThat(d.getTypeInitialization().getBasicTypeExpression(),
                       anyOf(instanceOf(TypeNameNode.class), instanceOf(ArrayTypeNode.class)));
        }
        assertThat(ds.get(1).getTypeInitialization().getBasicTypeExpression(), instanceOf(ArrayTypeNode.class));
        ArrayTypeNode<? extends BasicTypeExpressionNode<?>> at = (ArrayTypeNode<? extends BasicTypeExpressionNode<?>>) ds
            .get(1).getTypeInitialization().getBasicTypeExpression();

        // ここでもjavaのジェネリックスの問題が起こる
        final List<?> elements = at.getElementTypeList();
        assertThat(((TypeExpressionNode<TypeNameNode>) elements.get(0)).getBasicTypeExpression().getString(),
                   equalTo("number"));
        assertThat(((TypeExpressionNode<TypeNameNode>) elements.get(1)).getBasicTypeExpression().getString(),
                   equalTo("string"));
